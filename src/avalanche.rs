...
impl AvalancheNode {
    /// Creates an empty Avalanche node builder.
    ///
    /// # Parameters
    /// - None.
    ///
    /// # Returns
    /// - `AvalancheNodeBuilder`: Builder for message bits and per-bit biases.
    ///
    /// # Expected Output
    /// - Returns an empty builder; no stdout/stderr output.
    pub fn builder() -> AvalancheNodeBuilder {
        AvalancheNodeBuilder::new()
    }
    ...
    pub fn message_bits_vec(&self) -> Vec<bool> {
        self.packed_message_bits.to_bools()
    }
}

/// Builder for an Avalanche node.
#[derive(Debug, Clone, Default)]
pub struct AvalancheNodeBuilder {
    message_bits: Vec<bool>,
    biases: Vec<f64>,
}

impl AvalancheNodeBuilder {
    /// Creates an empty Avalanche node builder.
    ///
    /// # Parameters
    /// - None.
    ///
    /// # Returns
    /// - `AvalancheNodeBuilder`: Empty node builder.
    ///
    /// # Expected Output
    ...
    /// Sets the message bits for the node.
    ///
    /// # Parameters
    /// - `message_bits`: Candidate message bits in little-endian order.
    ///
    /// # Returns
    /// - `AvalancheNodeBuilder`: Updated builder.
    ///
    /// # Expected Output
    /// - Returns the updated builder; no stdout/stderr output.
    pub fn message_bits(mut self, message_bits: Vec<bool>) -> Self {
        self.message_bits = message_bits;
        self
    }

    /// Sets the per-bit biases for the node.
    ///
    /// # Parameters
    /// - `biases`: Bias values aligned with `message_bits`.
    ///
    /// # Returns
    /// - `AvalancheNodeBuilder`: Updated builder.
    ///
    /// # Expected Output
    /// - Returns the updated builder; no stdout/stderr output.
    pub fn biases(mut self, biases: Vec<f64>) -> Self {
        self.biases = biases;
        self
    }

    /// Builds the node, validating that bits and biases have matching non-zero widths.
    ///
    /// # Parameters
    /// - None.
    ///
    /// # Returns
    /// - `Result<AvalancheNode, AvalancheError>`: Node on success or a width error.
    ///
    /// # Expected Output
    /// - Returns an in-memory node; no stdout/stderr output.
    pub fn build(self) -> Result<AvalancheNode, AvalancheError> {
        if self.message_bits.is_empty() || self.message_bits.len() != self.biases.len() {
            return Err(AvalancheError::InconsistentBitWidth);
        }
        Ok(AvalancheNode::new(self.message_bits, self.biases))
    }
}
...
impl Avalanche {
    /// Creates an empty Avalanche reducer builder.
    ///
    /// # Parameters
    /// - None.
    ///
    ...
    /// # Expected Output
    /// - Returns the builder; no stdout/stderr output.
    pub fn builder() -> AvalancheBuilder {
...
mod tests {
    ...
    #[test]
    fn test_avalanche_builder_constructs_reducer() {
        let candidates = vec![
            AvalancheNode::builder()
        ...
        let avalanche = super::Avalanche::builder()
            .candidates(candidates)
            .expect("candidate conversion failed")
            .collect_scores(true)
            .build()
            .expect("avalanche build failed");

        let result = avalanche.execute().expect("avalanche execution failed");

        assert_eq!(result.node.message_bits_vec(), vec![true, false]);
        assert_eq!(result.level_pair_counts, vec![1]);
    }
