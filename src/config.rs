...
// Configuration schema and loader for fuel Avalanche settings.
use std::{
    error::Error,
    fs,
    path::{Path, PathBuf},
...
/// Top-level configuration for fuel Avalanche features.
...
pub struct Config {
    ...
    /// Source configuration path recorded by the loader for resolving relative file references.
    #[serde(skip)]
...
/// Loads the JSON config from disk, falling back to defaults if missing.
///
/// # Parameters
...
/// - `relative_or_absolute_path`: Path string that may be relative or absolute.
///
/// # Returns
/// - `PathBuf`: Resolved filesystem path to the requested path.
///
/// # Expected Output
